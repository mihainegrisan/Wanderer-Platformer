...
public class CharacterController2D : MonoBehaviour {
    ...
    // private void Update() {
    //     isHoldingJumpButton = Input.GetKey(KeyCode.Space);
    // }
    ...
    private void BetterJump() {
        if (m_Rigidbody2D.velocity.y < 0) {
            m_Rigidbody2D.velocity += Vector2.up * Physics2D.gravity * ((fallMultiplier - 1) * Time.fixedDeltaTime);
        } else if (m_Rigidbody2D.velocity.y > 0 && !Input.GetKey(KeyCode.Space)) {
            m_Rigidbody2D.velocity += Vector2.up * Physics2D.gravity * ((lowJumpMultiplier - 1) * Time.fixedDeltaTime);
        }   
    }
    ...
    public void Move(float move, bool crouch, bool jump) {
        ...
        if (m_Grounded && jump) {
            ...
            //m_Grounded = false;
            //m_Rigidbody2D.AddForce(Vector2.up * (m_JumpForce * Time.fixedDeltaTime), ForceMode2D.Impulse);
